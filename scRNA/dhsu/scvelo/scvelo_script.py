...
"""
@Introduce  :
@File       : scvelo_script.py
@Author     : ryrl
@email      : 18762739367@163.com
@Time       : 2022/10/26 12:57
"""

import anndata
import scvelo_script as scv
import pandas as pd
import numpy as np
...
if __name__ == '__main__':
    ...
    #
    # # umap = umap[np.isin(umap["Cell_ID"], adata_index["Cell_ID"])]  # TODO no intersection, remove the suffix of the adata_index
    #
    ...
    #
    # adata.obsm[['umap']] = umap_ordered.values  # TODO the shape of the adata is not same
    #
    ...
    umap = pd.merge(adata_index, umap, on='Cell_ID', how='left')

    umap = umap.iloc[:, 1:]

    adata.obsm['umap'] = umap.values

    cell_clusters = cell_clusters.rename(columns={cell_clusters.columns[0]: 'Cell_ID'})
    cell_clusters = pd.merge(adata_index, cell_clusters, on='Cell_ID', how='left')
    cell_clusters = cell_clusters.iloc[:, 1:]
    adata.obs['clusters'] = cell_clusters.values

    pd.DataFrame(adata.var.index.duplicated()).value_counts()
    ...
    scv.pl.proportions(adata,
                       dpi=300,
                       save='/public/workspace/ryrl/projects/singleCellDataSets/scRNA/others/dhsu/figures/test.pdf') # save the figure, chang to your own directory
