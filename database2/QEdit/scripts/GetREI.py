...
import sys, os, time, math, random, getopt, operator, string, errno, argparse
from collections import Counter
try: import pysam
except: sys.exit('Pysam module not found.')

parser = argparse.ArgumentParser(
	description= 'The overall editing calculated at recoding positions rediding in coding protein genes')

parser.add_argument('--input', '-i', required=True,
					metavar='input file', dest='redi',
					help='Table file by REDItools')
parser.add_argument('--recode', '-r', required=True,
					metavar='recode', dest='recode',
					help='RECODING positions')
...
allchrs=table.contigs  # 返回染色体名称列表
...
for i in f:  # i为f中的每一行
	...
	allrec+=1
	chr=l[0]  # 染色体名
	cc=(int(l[1])-1,int(l[1]))  # 位置坐标
	gene=l[10]  # 取基因名
	la=((l[12].split(',')[0]).split(':')[-1]).split('.')[1]  # 将第十二列分割
	aa=la[0]+la[-1]
